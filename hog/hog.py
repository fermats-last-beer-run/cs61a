...
def free_bacon(score):
    ...
    # BEGIN PROBLEM 2
    """ Calculates the alternating difference of the score's digits
        >>> 64
        (6 - 4)
        >>> 8000
        (8 - 0 + 0 - 0)
    ...
    def score_arithmetic(score):
        if num_of_places(score) % 2 == 0:
            alt_count = 1
        else:
            alt_count = 0
        ...
        return abs(arith_sum)

    return 1 + score_arithmetic(score**3)
    # END PROBLEM 2
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    p1_prev_turn = 0
    # BEGIN PROBLEM 5
    # TODO: Clean this up. Higher order function? Not sure.
    while score0 < goal and score1 < goal:
        if who == 0:
            rolls = strategy0(score0,score1)
            p0_turn = take_turn(rolls,score1,dice)
            # Feral Hogs Rule
            if feral_hogs:
                if abs(p0_prev_turn - rolls) == 2:
                    score0 = score0 + 3
            score0 = score0 + p0_turn
            p0_prev_turn = p0_turn           
        elif who == 1:
            rolls = strategy1(score1,score0)
            p1_turn = take_turn(rolls,score0,dice)
            #Feral Hogs Rule
            if feral_hogs:
                if abs(p1_prev_turn - rolls) == 2:
                    score1 = score1 + 3
            score1 = score1 + p1_turn
            p1_prev_turn = p1_turn
        else:
            print("ERROR")

        # Swap scores if excitement is met
